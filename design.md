  ...
  components: [
    {
      ...
      // Optional `isDefault: true` flag marks per-component built-in
      // entries — the records that carry this flag are the canonical
      // built-ins (`initialize`, `resendLastReceivedMessage`). See
      // the per-component built-ins bullet at the end of §3.1.
      localFunctions: [ {name, description, steps, isDefault?} ],
...
- **Per-component built-ins.** Two built-ins live at component scope rather than device scope, both stored in every component's `localFunctions` array with `isDefault: true`. Both follow the UI conventions of the device-wide built-ins (muted-italic in the list, locked from rename and deletion), and both are handled by the same set of helpers — but they differ in how much of their record is user-mutable:

  | Built-in | `stepsEditable` | Steps panel binding | Save policy |
  | --- | --- | --- | --- |
  | `initialize` | `true` | Binds normally; user fills in startup logic | Stripped when steps empty; persisted as `{name, steps}` when non-empty |
  | `resendLastReceivedMessage` | `false` | Hidden when this row is selected | Always stripped (wholly reconstructable) |

  `resendLastReceivedMessage` is a fixed-semantics FCM mechanism: it re-injects the last received message at the head of the message queue. It exists in Stadiæ as a named entity so transition action text can reference it (e.g. `` `resendLastReceivedMessage` ``) the same way it references any other local function — but the entity itself has no user-editable surface, hence the Steps panel suppression and the always-strip save policy.
...
| `*` + `H` + non-Yes/No messages           | Wildcard (ANY) into history  |
...
**Per-component built-ins.** The `localFunctions` array carries two built-ins by default — `initialize` and `resendLastReceivedMessage` (see §3.1) — and they plug into the panel differently:

- `initialize` binds like any other function: selecting it opens the Steps editor on its `steps` field, typing live-saves, and the same session-coalescing rules apply. The dialog-driven Edit path is the only thing that's blocked — the per-list `✎` button disables, and `editSelectedLocalFunction` defensively returns early when the selection is the default — because name and description are fixed for built-ins.
- `resendLastReceivedMessage` does not bind to the panel at all. `buildStepsPanel` checks `isDefaultLocalFunctionStepsEditable(lf)` and treats a fixed-step default the same way it treats "selection points at a non-existent function" — the panel and its top resize handle take the `.hidden` class and the list-box above reclaims the column. The user can still see the row (muted-italic, with its canonical description) but there is no editing surface for steps that have no user-authored content.

The Delete path follows the same convention as `deleteSelectedHandler` for both built-ins: a wholly-default selection disables the `−` button; a mixed selection enables delete and the leaf filters defaults out.
...
**Live-preview CSS.** The overlay's default `white-space: pre-wrap` faithfully echoes the textarea's literal whitespace, which is wrong for block-level Markdown output (paragraph margins, list indentation). When attached with `{ markdown: true }`, the overlay gets a `.markdown` class that flips white-space to `normal` and adds the `.live-md-h*`/`.live-md-p`/`.live-md-list` style rules. The `attachLiveBacktickRendering` helper picks `renderLiveMarkdown` over `renderLiveBackticks` based on the same flag. Four call-sites currently pass it: the Action panel, the Steps panel, the Device Specification panel, and the Description panel (which binds to either a component or a handler). The type-definitions modal's right-pane preview also switches to `renderLiveMarkdown` for parity with the export.
...
**Diagram linkification.** Inlined SVG gives the spec one more affordance for free: every shape in the diagram can be a navigation target. After `fetchDiagramAsInlineSVG` returns the cleaned SVG, the HTML spec builder passes it through `linkifyDiagramSVG(svg, ctx)`. The function walks the PlantUML output for `<g id="elem_<name>">` openers (which is how PlantUML renders each declared element), resolves `<name>` against the model, and wraps the group in `<a href="#<anchor>">…</a>` using the same anchor builders the inline backtick references use. The context object selects the resolution rule set: `kind: "device"` resolves names as components / handlers / interfaces (so the device diagram and the per-component context diagram both work); `kind: "component"` resolves names as states or choice points within the named component (`CP_` prefix denotes a choice point in PlantUML's element id). Synthetic nodes (`START`, `ANY`), built-in entities (`Timer`/`Logical`/`TimerHandler`), multiplied-component wrappers (`<comp>xN`), and anything else that doesn't match a model entity is left untouched — wrapping a stray id in `<a href="#nonexistent">` would create dead links. The wrap uses SVG2's plain `href` attribute (no `xlink:` namespace). The CSS rule `.diagram a[href] { cursor: pointer; }` provides the only visual affordance: a pointer cursor on hover, no underline or highlight. The diagram's own shape and label already make the target obvious.
...
The registry is three methods today:
...
- `get_diagram({scope, component?})` returns `{ svg, scope, component? }`. With `scope: "device"` the generator is `generateComponentDiagramPlantUML`; with `scope: "component"` it is `generatePlantUML`, the active-index swap-and-restore idiom (see the spec exporter, §15) used to render an arbitrary component without disturbing what the user is looking at. Both paths pass `withSelection: false` so the SVG is rendered clean — the bridge surface stays free of UI selection state. The PlantUML source is fetched as SVG from the user's configured server via `fetchPlantUMLSVG`, a thin wrapper around `plantUMLSVGURL` + `fetch().text()` that translates network failures into structured errors. Read-only: does not touch `Model.version`, does not push a history entry, does not mutate anything. No caching — the spec exporter's "every render is fresh" discipline applies here too.
...
4. **Display-label conventions.** Display labels (`deviceDisplayName`, every `displayName`, choice-point `question`) carry line breaks as the literal two-character marker `\n` — backslash plus the letter `n`. A raw newline character (U+000A) in one of these fields is rejected with a hint that points at the marker. Catches the common "I used a real newline instead of the marker" mistake that the editor's PlantUML generator otherwise silently mis-renders. Other free-text fields (every `description`, transition `action`, local-function `steps`, type `specification`, `deviceSpecification`) are full-prose surfaces and use real newlines normally — they are *not* checked here.
5. **Message references.** Each message references a known interface (built-in or user-defined); message names unique within their interface.
6. **Per-component checks.** Reserved-name local functions; transition source/target validity (existing state, `CP_<existing-cp>`, `START`/`*` for source; existing state, `CP_<existing-cp>`, `[H]` for target); at-most-one-START rule; the START transition's empty-messages requirement; per-message ANY-message-wildcard placement (sole-on-self-transition); duplicate `(source, message)` pairs within one component.
7. **Connection references.** `connections` and `handlerConnections` reject built-in interfaces and unknown endpoints; `handlerCalls` rejects unknown components/handlers.
8. **Multiplication invariant.** At most one component carrying a non-empty `multiplication` may be wired to any given interface.
...
- **Add a per-component built-in** (like `initialize` or `resendLastReceivedMessage` in `localFunctions`): start from the documentation-only-field recipe above, then layer on the built-in machinery. The current `localFunctions` worked example uses the *list-of-defaults* shape: a single `getDefaultLocalFunctions()` helper returns every default and its per-default policy flags (currently `stepsEditable`); `isDefaultLocalFunction(lf)` is the "any default?" predicate; `ensureDefaultLocalFunctions(component)` is the idempotent injector that loops over every default. To add another local-function built-in, append a record to `getDefaultLocalFunctions()` and decide whether its mutable surface is user-editable — the rest (`isDefaultLocalFunction`, the muted-italic list class, the `selectionIsOnlyDefaultLocalFunctions` toolbar gate, the save-path strip branch, the load-path re-injection) is shared. If a built-in has *no* user-editable surface (like `resendLastReceivedMessage`), it should additionally suppress its editing panel — see §10.7 for the worked Steps-panel example. For deletion semantics, follow `deleteSelectedHandler`'s convention: mixed selections (default + user records) keep delete enabled and the leaf filters defaults out before mutating, so the user can still bulk-delete around the built-ins.

  For an entirely *new* per-component field that wants its own set of built-ins (not yet exercised in the codebase), follow the same shape as the local-functions recipe: a `getDefault{Things}()` source-of-truth helper, an `isDefault{Thing}(record)` predicate, an `ensureDefault{Things}(component)` idempotent injector, an `onlyDefault{Things}()` predicate to gate the per-list `✎`/`−` buttons, a `if (isDefault{Thing}(record)) li.classList.add("default-item")` line in the list builder, a defensive return in the leaf edit/delete handlers for the keyboard path, a save-path branch that strips wholly-reconstructable defaults (and emits a minimal record for partially-mutable ones), and a call to `ensureDefault{Things}` from `makeEmptyComponent` (so every creation path picks them up) and from the load path (so older files are upgraded). The `localFunctions` family is the recommended template — see §3.1 for the model layer and §10.7 for the panel-binding details.
