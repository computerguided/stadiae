  ...
  components: [
    {
      ...
      // Optional `isDefault: true` flag marks the per-component
      // built-in `initialize` entry — the only record that carries
      // this flag in this array. See the per-component built-ins
      // bullet at the end of §3.1.
...
- **Per-component built-ins.** One additional built-in lives at component scope rather than device scope: every component's `localFunctions` array carries an `initialize` entry with `isDefault: true`. It follows the same UI conventions as the device-wide built-ins (muted-italic in the list, locked from rename and deletion) but with a key difference: its `steps` field is user-editable. The per-component variant is centralised in three small helpers — `makeDefaultInitializeFunction()` (the canonical record), `isDefaultLocalFunction(lf)` (predicate), and `ensureDefaultLocalFunctions(component)` (idempotent injector). `makeEmptyComponent` calls `ensureDefaultLocalFunctions` before returning, so every creation path (initial Model, `resetModel`, `addComponent`, post-deletion fallback) gets the default for free; the file-load path runs it on every loaded component, which both re-injects a missing record and upgrades a pre-existing user `initialize` to carry the `isDefault` flag while preserving its `steps`. The save path mirrors the device-wide convention but adapts it: when `steps` are empty the entry is stripped entirely; when non-empty, only `{name, steps}` is persisted (description and `isDefault` are reconstructable on load). This pattern is the recommended template for any future per-component built-in — see the matching extension recipe in §17.
...
**Built-in `initialize`.** The per-component default `initialize` local function (see §3.1) plugs into the panel like any other function: selecting it binds the Steps editor to its `steps` field, typing live-saves, and the same session-coalescing rules apply. The dialog-driven Edit path is the only thing that's blocked — the per-list `✎` button disables, and `editSelectedLocalFunction` defensively returns early when the selection is the default — because name and description are fixed for built-ins. The Delete path follows the same convention as `deleteSelectedHandler`: a wholly-default selection disables the `−` button; a mixed selection enables delete and the leaf filters the default out.
...
- **Add a per-component built-in** (like `initialize` in `localFunctions`): start from the documentation-only-field recipe above, then layer on the built-in machinery — three small helpers (`makeDefault{Thing}`, `isDefault{Thing}`, `ensureDefault{Things}` — the last is the idempotent injector that preserves any user-authored mutable fields if a same-named record is already present), a call to `ensureDefault{Things}` from `makeEmptyComponent` so every creation path picks it up, another from the load path so older files are upgraded, a save-path branch that strips wholly-default entries and emits a minimal record for non-default ones (drop fields that can be reconstructed from the helper), an `if (isDefault{Thing}(record)) li.classList.add("default-item")` line in the list builder, an `onlyDefault{Things}()` predicate to gate the per-list `✎`/`−` buttons, and a defensive return in the leaf edit/delete handlers for the keyboard path. For deletion semantics, follow `deleteSelectedHandler`'s convention: mixed selections (default + user records) keep delete enabled and the leaf filters defaults out before mutating, so the user can still bulk-delete around the built-in. The `initialize` local function is the worked example — see §3.1 for the model layer and §10.7 for the panel-binding details.
