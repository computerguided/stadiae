...
Per-component, every component carries two built-in local functions: `initialize` (the conventional slot for the component's startup logic, with user-editable *Steps*) and `resendLastReceivedMessage` (an FCM mechanism that re-injects the last received message at the head of the message queue, with fixed semantics — name, description, and body are all canonical, so the Steps panel hides when this row is selected). Both follow the same muted-italic / locked-from-rename-and-deletion convention as the device-wide built-ins. Saved files don't carry either built-in unless `initialize` has user-typed steps (in which case just `{name, steps}` round-trips); both are re-injected on load.
...
- **Local functions.** Per-component reusable action snippets — name plus multi-line description/steps. Reference them by name from transition action text to avoid repeating common step sequences. Take no parameters (they access the component's state variables by closure) and don't reach the generated PlantUML. Appear as the final section of each component's chapter in the spec export, with multi-line descriptions preserved. Each component starts with two built-in entries — `initialize` (steps user-editable) and `resendLastReceivedMessage` (an FCM mechanism with fixed semantics) — both locked from rename/deletion.
- **Component multiplication.** An optional free-text marker on each component (typical values: `N`, `NUM`, `i`) flagging that several instances of the component exist at runtime. When set, the device diagram renders the component inside a wrapping rectangle labelled with the value (so `N` shows as `N`); when empty, the component renders plain. Every interface wired to a multiplied component is wrapped in the same rectangle, with its visible label suffixed `[]` to mark the multiplicity. Documentation-only beyond that visual: state machines, connections, and the spec export's tables are unchanged. Edited from the component's *Edit* dialog. An invariant is enforced: at most one multiplied component per interface — adding a connection that would violate it (or setting a multiplication that would create one) is refused with a clear message. Handlers do not have this field.
...
- **Markdown formatting in long-form fields.** The Device Specification, component and handler descriptions, transition Action notes, local-function Steps, and type-definition Specification fields honour a curated Markdown subset (headings, bullet/numbered lists, paragraphs, `**bold**`, `*italic*`) in both the live preview and the spec export. Backtick references work alongside, so a `**bold mention of `Idle`**` renders as bold text with a resolved link inside. Other description fields stay plain.
...
- **WIND** — *Wireless Interactive Networked Display*. A hardware-oriented device with three components (`ServerConnector`, `DeviceStatusController`, `UiController`), five handlers (`WirelessTransceiver`, `PeripheralHandler`, `TouchDisplay`, `AudioHandler`, `ProximityDetector`), seven interfaces, nineteen messages, and six type definitions. Exercises the radio / peripheral / display / audio / proximity-sensor families of handler. [View the rendered specification](https://htmlpreview.github.io/?https://raw.githubusercontent.com/computerguided/stadiae/refs/heads/main/examples/WIND.html) · [download the model](examples/WIND.json).
- **DCSC** — *Doors Controlling System Controller*. A control-flow-oriented device with three components (`Administrator`, `DeviceController`, `DoorController`), three handlers (`ConfigurationDatabase`, `BackendInterface`, `SensorHandler`), five interfaces, twenty-eight messages, and five type definitions. Exercises configuration storage, a backend integration, and per-door sensor management. [View the rendered specification](https://htmlpreview.github.io/?https://raw.githubusercontent.com/computerguided/stadiae/refs/heads/main/examples/DCSC.html) · [download the model](examples/DCSC.json).
