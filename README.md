...
Per-component, every component carries one built-in `initialize` local function — the conventional slot for the component's startup logic. It follows the same muted-italic / locked-from-rename-and-deletion convention as the device-wide built-ins, with one twist: its *Steps* are user-editable. Empty `initialize` steps are stripped from saved files (re-injected on load); a non-empty body is preserved.
